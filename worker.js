...
const SW = `const C='playa-camiones-v1';self.addEventListener('install',e=>{self.skipWaiting();e.waitUntil(caches.open(C).then(c=>c.addAll(['/','/manifest.webmanifest','/icon.svg']))) });self.addEventListener('activate',e=>e.waitUntil(self.clients.claim()));self.addEventListener('fetch',e=>{if(e.request.method!=='GET')return;e.respondWith(fetch(e.request).catch(()=>caches.match(e.request)))})`;
...
export default {
  async fetch(req, env) {
    try {
      ...
      if (u.pathname === '/sw.js' && req.method === 'GET') return new Response(SW,{headers:{'content-type':'text/javascript;charset=UTF-8','cache-control':'no-cache'}});
