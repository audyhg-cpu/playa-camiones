...
function nextPlaces(){const filas=[],resultados=[];for(let f=16;f<=32;f++)if(S.filas[f]?.producto&&S.filas[f].producto!=='Vacío')filas.push(f);filas.sort((a,b)=>(S.filas[b]?.activada_en||0)-(S.filas[a]?.activada_en||0)||a-b);for(const f of filas){const sentido=S.filas[f]?.sentido==='izquierda'?'izquierda':'derecha',orden=sentido==='izquierda'?[5,4,3,2,1]:[1,2,3,4,5];for(const p of orden){const x=S.lugares[f]?.[p]||{};if(!x.ocupado&&!x.bloqueado){resultados.push({fila:f,pos:p,producto:S.filas[f].producto});break}}}return resultados}
function updateGuide(){const lugares=nextPlaces(),habilitada=Object.values(S.filas).some(x=>x.sector==='post'&&x.producto&&x.producto!=='Vacío');$('guide').textContent=lugares.length?(lugares.length===1?'Próximo: ':'Próximos: ')+lugares.map(n=>n.producto+' · Fila '+n.fila+' · Lugar '+n.pos).join('  •  '):(habilitada?'Sin lugares disponibles':'Sin fila habilitada')}
